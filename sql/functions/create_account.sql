...
BEGIN
    ...
    VALUES (
        v_user_id,
        jsonb_build_object('app_metadata', jsonb_build_object('role', v_role))
    )
    ON CONFLICT (user_id) DO UPDATE
    SET raw_user_meta_data =
        COALESCE(auth.user_profiles.raw_user_meta_data, '{}'::jsonb)
...
--    LIMIT 1;
    ...
    ---------------------------------------------------------
    -- Normaliser le nom d'organisation pour un individu
    ---------------------------------------------------------
    IF p_is_individual
       AND (p_organization_name IS NULL OR btrim(p_organization_name) = '') THEN
        p_organization_name := p_first_name || ' ' || p_last_name;
    END IF;
